...
function gitAddedDates(){const dates=new Map();try{const raw=execFileSync('git',['log','--reverse','--diff-filter=A','--format=@@%aI','--name-only','--','downloads'],{encoding:'utf8',stdio:['ignore','pipe','ignore']});let current='';for(const line of raw.split(/\r?\n/)){if(line.startsWith('@@')){current=line.slice(2).trim();continue}const file=line.trim().replace(/\\/g,'/');if(current&&file.startsWith('downloads/')&&!dates.has(file))dates.set(file,current)}}catch{}return dates}
...
function discover(){const old=fs.existsSync(CATALOG)?readJson(CATALOG):{files:[]};const addedDates=gitAddedDates();const oldByProv=new Map((old.files||[]).map(x=>[x.provenance,x]));const oldByFile=new Map((old.files||[]).map(x=>[x.filename,x]));const out=[];const seenFiles=new Set();for(const name of fs.readdirSync(ROOT).filter(x=>x.endsWith('.json')&&x!=='catalog.json').sort()){
