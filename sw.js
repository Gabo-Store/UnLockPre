...
self.addEventListener('fetch',e=>{
  ...
  e.respondWith(caches.match(e.request).then(cached=>cached||fetch(e.request).then(r=>{if(r.ok&&['document','script','style','image'].includes(e.request.destination)){const x=r.clone();caches.open(CACHE).then(c=>c.put(e.request,x))}return r}).catch(()=>caches.match('./index.html'))));
});
